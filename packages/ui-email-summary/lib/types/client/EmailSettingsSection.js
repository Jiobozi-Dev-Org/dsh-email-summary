...
export function EmailSettingsSection({ api, t }) {
    ...
    const onReportNow = useCallback(() => {
        ...
        void api.reportNow().then((result) => {
            setReporting(false);
            if (result.ok)
                setReportMessage(`${t('settings.reportNowSent')}（${result.count} 个会话）`);
            else
                setReportMessage(`${t('settings.reportNowFailed')}：${result.error ?? ''}`);
            refreshReportStatus();
        });
